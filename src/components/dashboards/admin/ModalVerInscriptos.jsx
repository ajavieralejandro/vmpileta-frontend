...
export default function ModalVerInscriptos({ turno, onClose, onRecargar }) {
  ...
  const cargarInscriptos = async () => {
    ...
    setLoading(true);
    try {
      const response = await inscripcionesAPI.getPorTurno(turnoId);

      // ✅ Esto es lo que te devuelve axios (según tus logs)
      console.log('[DEBUG] response.data:', response?.data);
      console.log('[DEBUG] response.data.data:', response?.data?.data);

      // ✅ RUTA REAL DEL ARRAY
      const lista = response?.data?.data?.inscripciones;

      console.log('[DEBUG] inscripciones raw:', lista);
      console.log('[DEBUG] isArray:', Array.isArray(lista), 'len:', Array.isArray(lista) ? lista.length : 'N/A');

      setInscriptos(Array.isArray(lista) ? lista : []);
    } catch (error) {
      console.error('[DEBUG] error cargarInscriptos:', error);
  ...
  useEffect(() => {
    console.log('[state] inscriptos updated len:', inscriptos.length, inscriptos);
  }, [inscriptos]);
  ...
  const darDeBaja = async (inscripcionId, nombreAlumno) => {
    ...
    try {
      ...
      console.error('[DEBUG] error darDeBaja:', error);
  ...
  const descargarExcel = async () => {
    ...
    try {
      ...
      // ✅ Usa tu service exportsAPI (viene con responseType blob)
      ...
      console.error('[DEBUG] error descargarExcel:', e);
